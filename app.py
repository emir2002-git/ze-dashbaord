...
st.dataframe(
    combined[
        ["Firm ID", "Firm Name", "Date", "Daily Revenue", "Avg Daily Revenue"]
    ],
    use_container_width=True
)
...
if selected == "All":
    ...
    name = latest["Firm Name"]
    date = latest["Date"]

    st.markdown(f"### Recommendations for **{name}** on {date}")
...
- **Raise prices by 3–5%** on your best-selling items to improve margins.
...
"""
        ...
        st.markdown(
            """
- **Maintain current pricing**, but keep an eye on competitors.
- **Invest in targeted marketing** (social ads or loyalty programs) to grow further.
...
# ── Export Yearly Banking Summary ────────────────────────────────────────────────
if st.button("📥 Download Yearly Summary CSV"):
    # aggregate yearly figures
    yearly = (
        pos
        .groupby(["Firm ID"])[["Revenue (KM)"]]
        .sum()
        .reset_index()
        .rename(columns={"Revenue (KM)": "Yearly Revenue"})
    )
    yearly = yearly.merge(firms[["Firm ID","Firm Name","Bank","Package"]], on="Firm ID")
